...
class iLink extends JTree
{
	...
	function _getLayouts($path, &$node)
	{
		...
		$folders = array($path.DS.'tmpl',$path.DS.'html');
        foreach($folders as $folder)		
		if (is_dir( $folder ))
		{			
            $foldername = basename($folder);
			$files = JFolder::files($folder, '(.php|.xml)$');
			$array = array();
			...
			if (count($files)) {
				foreach ($files as $file)
				{
					if (strpos($file, '_') === false) {
						// Load view metadata if it exists
						$layout = JFile::stripext($file);
						$xmlpath = $path.DS.$foldername.DS.$layout.'.xml';                        
						if (JFile::exists($xmlpath)) {
						...
						if ( $data && $data->name() == 'menu') {                            
							$params = $data->getElementByPath('params');
							$query  = array('url[view]='.basename($path),'url[option]=com_'.$this->_com,'url[layout]='.$layout);
							foreach($params->children() as $param) {
								$query[] = 'url['.$param->attributes('name').']'.'='.$param->attributes('value');
							}
							$url = implode('&amp;', $query);
						}*/
						...
						if ($data) 
                        {
                            if ( $params = $data->getElementByPath('url') ) 
                            {
                                $str   = $url;
                                $url   = array();
                                parse_str(html_entity_decode($str), $url);
                                foreach($params->children() as $param) {
                                ...
                                $url['template'] = pick($layout,'default');
                                $url = htmlentities(urldecode(http_build_query($url)));
                            }
							if ($data->attributes('hidden') != 'true') 
                            {
								$m = $data->getElementByPath('message');
