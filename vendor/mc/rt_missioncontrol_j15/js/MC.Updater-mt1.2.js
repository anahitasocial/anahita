...
var Updater = {
	...
	init: function(){
		...
		Updater.ajax = new Request({url: 'index.php?process=ajax&model=updater', onRequest: Updater.request, onSuccess: Updater.success});
	...
	success: function(r){
		...
		var tmp = new Element('div').set('html', r);
		tmp = tmp.getFirst();
		var box = document.id(document.body).getElement('.mc-update-check');
