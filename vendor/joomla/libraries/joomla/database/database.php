	...
	static public function &getInstance( $options	= array() )
	{
		...
		if (empty($instances[$signature]))
		{
			...
			if (file_exists($path)) {
				...
				$error = JError::raiseError( 500, AnTranslator::_('Unable to load Database Driver:') .$driver);
				return $error;
			}
			...
			if ( $error = $instance->getErrorMsg() )
			{
				...
				$error = JError::raiseError( 500, AnTranslator::_('Unable to connect to the database:') .$error);
				return $error;
			}
