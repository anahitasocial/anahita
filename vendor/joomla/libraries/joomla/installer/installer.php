...
class JInstaller extends JObject
{
	...
	function install($path=null)
	{
		...
		$enforce	= $root->attributes('enforce');
		$rootName	= $root->name();
		$config		= &JFactory::getConfig();
	    
		if (version_compare($version, '1.5', '<')) 
		{
			$this->abort(JText::_('Extensions for version less than 1.5.0 are no longer supported.'));
			return false;
		}		 
		else if ( $enforce && version_compare($version, Anahita::getVersion(), '>') ) {
			$this->abort(JText::_('This extension requires Anahita to upgrade to version '.$version.'. Visit <a href="http://anahitapolis.com">http://anahitapolis.com</a> for the upgrade package.'));
			return false;			
		}
		...
		$ret     = true;
		$adapter =& $this->_adapters[$type];
		dispatch_plugin('installer.onBeforeInstallExtension',array(&$this, &$adapter, &$ret), JDispatcher::getInstance());		
		
		if ( $ret === false ) {
		    $this->abort(JText::_('Installation Failed'), $type);
		    return false;
		}
		...
		dispatch_plugin('installer.onAfterInstallExtension',array(&$this, &$this->_adapters[$type], &$ret), JDispatcher::getInstance());
	...
	{
	    dispatch_plugin('installer.onSetupInstall',array(&$this, $type), JDispatcher::getInstance());
		...
		$ret     = true;
		$adapter =& $this->_adapters[$type];
		dispatch_plugin('installer.onBeforeUnInstallExtension',array(&$this, &$adapter, $identifier, $cid, &$ret), JDispatcher::getInstance());				
		
        if ( $ret === false ) {
		    $this->abort(JText::_('Uninstall Failed'), $type);
		    return false;
		}
		
		if (is_object($this->_adapters[$type])) {
			$ret = $this->_adapters[$type]->uninstall($identifier, $cid);
		}
		
		dispatch_plugin('installer.onAfterUnInstallExtension',array(&$this, &$this->_adapters[$type], $identifier, $cid, &$ret), JDispatcher::getInstance());
