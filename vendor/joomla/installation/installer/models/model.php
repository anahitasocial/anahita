...
class JInstallationModel extends JModel
{
	...
	function makeDB($vars = false)
	{
		...
		if (!$DBcreated)
		{
			...
			$dbscheme = 'sql'.DS.$type.DS.'install.sql';
            ...
            $dbscheme = 'sql'.DS.$type.DS.'schema.sql';
	...
	function preInstall()
	{
		...
		$phpOptions[] = array (
			'label' => '- '.JText::_('MySQLi support'),
			'state' => (function_exists('mysqli_connect')) ? 'Yes' : 'No'
		);
		
		$phpOptions[] = array (
			'label' => '- '.JText::_('GD library support'),
			'state' => extension_loaded('gd') ? 'Yes' : 'No'
		);
	...
	function saveConfig()
	{
		...
		if ( empty( $forced['helpurl'] ) ) {
			$vars['helpurl'] = 'http://www.anahitapolis.com/help/resources';
		} else {
